...
def calcIndex(satellite, index_name, year, region, clip):
    ...
    if index_name == "NDVI":
        index_image = image.normalizedDifference([nir_band, red_band]).rename('NDVI')
    elif index_name == "NDWI":
        index_image = image.normalizedDifference([blue_band, nir_band]).rename('NDWI')
    elif index_name == "SAVI":
        index_image = image.expression(indexes["SAVI"], {
...
if selected_year and sat and roi:
    ...
    if check_index:
        ...
        # Display map and index statistics
        with row1_col1:
            Map.to_streamlit(height=600)

        # Display statistics in a separate column
        with row2_col1:
