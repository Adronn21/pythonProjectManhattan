...
def main():
    ...
    datasets = {
        ...
        'MODIS': {
            ...
            # 0-Red, 1-Blue, 2-Green, 3-NIR, 4-Red Edge(or red)
    ...
    roi = None
    index_layers = []  # List to store index layers
    ...
    with row1_col2:
        ...
        if check_index:
            ...
            # Button to add index layer
            if st.button('Add Index Layer'):
                if selected_year is not None and sat is not None and roi is not None:
                    # Calculate index
                    index_layer = calc_index(sat, index_name, selected_year, roi, clip)

                    # Add index layer to the map
                    Map.addLayer(index_layer,
